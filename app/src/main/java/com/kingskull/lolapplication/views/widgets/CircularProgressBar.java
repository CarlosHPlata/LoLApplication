package com.kingskull.lolapplication.views.widgets;
...
public class CircularProgressBar extends ProgressBar {
    ...
    public void init(AttributeSet attrs, int style){
        ...
        float size = a.getDimension(R.styleable.CircularProgressBar_cpb_titleTextSize, 100.0f);
        if (size != 0){
            mTitlePaint.setTextSize(size);
        }

        size = a.getDimension(R.styleable.CircularProgressBar_cpb_subtitleTextSize, 100.0f);
        if (size != 0){
        ...
        mTitlePaint.setTypeface(Typeface.create("Roboto-Bold", Typeface.NORMAL));
